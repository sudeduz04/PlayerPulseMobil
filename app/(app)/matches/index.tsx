...
const renderItem = ({ item }: { item: Match }) => <MatchListItem match={item} />;
...
export default function MatchesListScreen() {
  ...
  const onNew = useCallback(() => router.push('/(app)/matches/new' as never), []);
  ...
  const ListHeader = useMemo(
    () => (
      <>
        ...
        />
        <SearchInput value={search} onChangeText={setSearch} placeholder="Rakip ara..." />
        {teams.length > 1 ? (
