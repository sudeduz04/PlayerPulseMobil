...
export async function listMyTrainings(): Promise<
  Paginated<TrainingPerformance>
> {
...
export async function getMyHealth(): Promise<MyHealth> {
  const { data } = await api.get("/my/health");
  return unwrap<MyHealth>(data);
}
